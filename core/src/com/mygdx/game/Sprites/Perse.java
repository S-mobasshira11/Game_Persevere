...
public class Perse extends Sprite {
   ...
   // private Array<FireBall> fireballs;   ///////////////////////////////////////////////
    ...
    public Perse(PlayScreen screen){
        ...
        for(int i=1; i<4; i++)
        {
            frames.add(new TextureRegion(screen.getAtlas().findRegion("weak_perse"),i*16,0,16,16));
        }
        perseRun = new Animation(0.1f,frames);
        frames.clear();
        for(int i=1; i<4; i++)
        {
            frames.add(new TextureRegion(screen.getAtlas().findRegion("strong_perse"),i*16,0,16,32));
        }
        bigPerseRun = new Animation(0.1f,frames);
        frames.clear();

        frames.add(new TextureRegion(screen.getAtlas().findRegion("strong_perse"),240,0,16,32));
        frames.add(new TextureRegion(screen.getAtlas().findRegion("strong_perse"),0,0,16,32));
        frames.add(new TextureRegion(screen.getAtlas().findRegion("strong_perse"),240,0,16,32));
        frames.add(new TextureRegion(screen.getAtlas().findRegion("strong_perse"),0,0,16,32));
        growPerse = new Animation(0.2f,frames);

        perseJump = new TextureRegion(screen.getAtlas().findRegion("weak_perse"),80,0,16,16);
        bigPerseJump = new TextureRegion(screen.getAtlas().findRegion("strong_perse"),80,0,16,32);

        perseStand = new TextureRegion(screen.getAtlas().findRegion("weak_perse"), 0 , 0 , 16 , 16);
        bigPerseStand = new TextureRegion(screen.getAtlas().findRegion("strong_perse"),0,0,16,32);

        perseDead = new TextureRegion(screen.getAtlas().findRegion("weak_perse"),96,0,16,16);
        definePerse();
   ...
   /* public void grow(){
    ...
    public void die() {     /////////////////////////////////////////////////////////////////////
        ...
        if (!isDead()) {
            ...
            PersevereGame.manager.get("audio/music/time_music.ogg", Music.class).stop();
            PersevereGame.manager.get("audio/sounds/die.wav", Sound.class).play();
            perseIsDead = true;
